...
select * from meslekler where soyisim='Can' order by maas desc;
...
select urun_adi, max(urun_miktari), min(urun_miktari) from magaza group by urun_adi order by min(urun_miktari);
