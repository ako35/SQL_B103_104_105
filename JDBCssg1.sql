...
select isim, sehir from musteriler where exists (select * from siparisler where urun_id='defter');
...
select * from musteriler where exists (select * from siparisler where musteriler.id=siparisler.musteri_id);

--4)Siparişi olmayan müşterileri listeleyiniz.(ilişkili)
select * from musteriler where not exists (select * from siparisler where musteriler.id=siparisler.musteri_id);

--5)Müşterilerden yaşı 18 den küçük varsa siparişini sil.(ilişkili)
delete from siparisler where exists (select * from musteriler where yas<18 and musteriler.id=siparisler.musteri_id);

--6)İstanbulda yaşayan müşteri varsa müşteri_id , sipariş verdiği ürün adı
-- ve tutarı listeleyiniz.
select musteri_id, urun_adi, tutar from siparisler where exists (select * from musteriler where sehir='İstanbul' and musteriler.id=siparisler.musteri_id);

--7)İstanbulda yaşayan müşteri varsa sipariş tutarından 50 br indirim
--yapınız ve tüm siparişleri listeleyiniz.
update siparisler set tutar=tutar-50 where exists (select * from musteriler where sehir='İstanbul' and musteriler.id=siparisler.musteri_id);

--8)Çanta siparişi veren müşteri varsa isminin yanına * işareti koyunuz ve 
--tüm müşterileri listeleyiniz.
update musteriler set isim=isim || '*' where exists (select * from siparisler where siparisler.musteri_id=musteriler.id and urun_adi='çanta');
